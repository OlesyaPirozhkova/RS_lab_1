...
Создать Новостной агрегатор

Сервис NewsFeed:
метод GetLatestNews(NewsRequest) для
получения потока последних новостей по
категории (Server streaming RPC).
...
файле weather.proto.
•Роль контракта. Этот файл является "единым источником
правды" для API. Он строго описывает:
• Какие сервисы доступны (WeatherService).
• Какие методы можно вызвать у каждого сервиса
(GetCurrentWeather, SubscribeToForecasts).
• Какие данные (сообщения) эти методы принимают
(CityRequest) и возвращают (WeatherResponse).

<img width="678" height="721" alt="image" src="https://github.com/user-attachments/assets/9023b2bc-40bb-44a2-b7cb-5604c08cbcfa" />
